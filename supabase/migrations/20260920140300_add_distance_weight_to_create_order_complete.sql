/*
  # Add delivery_distance_km / delivery_weight_kg to create_order_complete

  1. Changes
     - `create_order_complete` gains two new trailing snapshot parameters,
       `p_delivery_distance_km numeric DEFAULT NULL` and
       `p_delivery_weight_kg numeric DEFAULT NULL`, written straight through
       to the new `orders.delivery_distance_km`/`delivery_weight_kg` columns
       (20260920140200_add_distance_weight_to_orders.sql). Same trust model
       as the rest of `delivery_fee`/`delivery_scope`: client-computed,
       stored as a snapshot for support/audit, not re-validated here.

  2. Important: DROP before CREATE
     - Adding a new parameter changes the function's argument-type identity,
       so `CREATE OR REPLACE FUNCTION` with extra trailing parameters does
       NOT replace the existing overload — it silently creates a second,
       distinct overload alongside it, breaking unqualified `GRANT`/PostgREST
       RPC resolution. Unlike prior migrations touching this function (which
       hard-coded the exact old signature to drop), this one drops
       dynamically by looking up whatever `create_order_complete` overload(s)
       currently exist in `pg_proc` — safer against the signature having
       drifted from what any single prior migration file recorded.
*/
