/*
  # Add p_insurance_fee to create_order_complete RPC

  1. Changes
    - Add `p_insurance_fee numeric DEFAULT 0` as a new, final parameter of
      `create_order_complete`, and insert it into the `orders.insurance_fee`
      column.
    - Appended at the end of the parameter list (after p_shipping_zip_code)
      so no existing caller (WhatsApp flow, which never passes it) breaks.
      The only current caller (src/lib/orderService.ts) uses named params,
      so ordering does not matter there either way.
    - Because the parameter list differs from the previous version,
      `CREATE OR REPLACE FUNCTION` does NOT replace the old function — it
      creates a second overload with the same name, which then makes the
      trailing `GRANT EXECUTE ON FUNCTION public.create_order_complete`
      ambiguous (error 42725). To avoid that, every existing overload of
      `create_order_complete` in the `public` schema is dropped first, so
      only the version defined below remains.
*/
