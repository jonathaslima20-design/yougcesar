/*
  # Add delivery_is_quote to orders and create_order_complete

  1. Contexto
    - Nova opção de entrega "Frete a Consultar" (DeliveryOption.quoteOnRequest
      em src/types/index.ts): não tem valor fechado, só disponível no fluxo
      de pedidos via WhatsApp — o valor real é combinado depois com o
      comprador. `delivery_fee` é gravado como 0 nesse caso, então o
      dashboard e as telas do comprador precisam de um sinal à parte para
      não mostrar "Grátis" onde na verdade é "A combinar".

  2. Modified Tables
    - `orders`
      - `delivery_is_quote` (boolean, not null, default false) - snapshot,
        no momento da criação do pedido, de `DeliveryOption.quoteOnRequest`
        da opção escolhida. Mesma convenção de `delivery_option` e
        `delivery_scope`: texto/valor livre, sem referência viva à
        configuração do lojista.

  3. Changes
    - Novo overload de `create_order_complete` com `p_delivery_is_quote
      boolean DEFAULT false` acrescentado como último parâmetro (mesma
      convenção append-only de 20260803120000_add_delivery_scope_to_orders.sql
      — todo overload existente é derrubado antes para o GRANT EXECUTE ficar
      sem ambiguidade). Chamadores existentes não são afetados: o parâmetro
      tem default false.
*/

ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS delivery_is_quote boolean NOT NULL DEFAULT false;
